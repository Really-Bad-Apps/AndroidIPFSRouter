...
const val KEY_PREFERRED_GATEWAY = "preferred_gateway"
...
fun getPreferredGateway(context : Context) : String? {
    return context.getSharedPreferences("ai.protocol.ipfs.filforge", Context.MODE_PRIVATE)
        .getString(KEY_PREFERRED_GATEWAY, "auto")
}
...
fun setPreferredGateway(context : Context, gateway : String) {
    context.getSharedPreferences("ai.protocol.ipfs.filforge", Context.MODE_PRIVATE)
        .edit().putString(KEY_PREFERRED_GATEWAY, gateway).apply()
}
